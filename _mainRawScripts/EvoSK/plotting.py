...
def getAva(qTraj_, fdelta, noDouble=False):
    ...
    for t in range(FId+1, FId_+1): #len(isEvent_)):
        if isEvent_[t]:
            ...
            size += sizeLow_[t]
            if noDouble:
                sizeID += list(np.arange(1024)[imins_[:, t].flatten()])
        else:
            if size > 0:
                sizes += [size]
                endTimes += [t]
                if noDouble:
                    sizes_noDouble += [len(set(sizeID))]
    ...
    if noDouble:
        return sizes_noDouble, startTimes, endTimes
...
def general(nAgents, J, numSeeds, startSeed, episodeLen, avgRTraj_, mTraj, rTraj,
    ...
    acf, acvf, m = computeAcf(mTraj, numLags=numLags, steadyIters=steadyIters)
    ...
    if cycleInset is not None:
        ...
        avgR_min = np.round(np.min(avgRTraj_[-20:, :]), 4)
        avgR_max = np.round(np.max(avgRTraj_[-20:, :]), 4)
        avgR_mean = np.round(np.average(avgRTraj_[-20:, :]), 4)
...
def Tevolution(nAgents, J, numSeeds, startSeed, episodeLen, tMid, 
    ...
    for seed_ in range(numSeeds):
        ...
        for pId in range(numTruepId):
            ...
            Tmin = tMin 
            Tmax = tMax 
            norm=SqueezedNorm(vmin=Tmin, vmax=Tmax, mid=.3*Tmax, 
                              s1=2, s2=2)
            ...
            ax_.set_title('{} Evo'.format(adaptStr[pId])) #'temp Evo')#countLowHigh[seed_])
            ...
            for k in kList: #kList: #np.linspace(1, K, numk): #[1, 100, 200, 300, 400, 500]:
                ...
                ax[rowID, kID].set_title('temp Dist Evo (window size={})'.format(windowSize))
        ...
        if nrandom or numOfi > 1:
            ...
            fminsBf_ = np.multiply(imins__, qs_[:, :-1])
            fminsAft_ = np.multiply(imins__, qs_[:, 1:])
        else:
            fminsBf_ = np.multiply(imins_[:, 1:], qs_[:, :-1])
        ...
        # works when numOfi = 1
        fminsBf_ = np.sum(fminsBf_, axis=0) / numOfis_
        fminsAft_ = np.sum(fminsAft_, axis=0) / numOfis_
        ...
        if isAvged > 1:
            ax[rowID, 1].set_title('AVG({}) f[imin] bf-aft Update'.format(isAvged))
        else:
